...
// ── LOCAL AUTH (Supabase Auth se conecta al publicar en Vercel) ──────────────
const auth = {
  register(name, email, pass) {
    const users = JSON.parse(localStorage.getItem("redgym-users") || "[]");
    if (users.find(u => u.email === email)) return { error: "Este correo ya está registrado" };
    const user = { id: Date.now(), name, email, pass, plan: "estandar", createdAt: new Date().toISOString() };
    users.push(user);
    localStorage.setItem("redgym-users", JSON.stringify(users));
  ...
  saveSession(user) {
    localStorage.setItem("redgym-session", JSON.stringify(user));
  },
  getSession() {
    try { return JSON.parse(localStorage.getItem("redgym-session")); } catch { return null; }
  },
  clearSession() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.bg, padding: 24, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", maxWidth: 360, margin: "0 auto", width: "100%" }}>
        ...
        <Btn onClick={handleLogin} style={{ width: "100%", padding: "14px", marginTop: 16 }}>Entrar</Btn>
      ...
      <div style={{ maxWidth: 400, margin: "0 auto", width: "100%" }}>
        ...
        {step === 1 ? (
          ...
          <>
            ...
            <Btn onClick={handleRegister} disabled={!selectedPlan} style={{ width: "100%", padding: "14px", marginTop: 12 }}>Continuar →</Btn>
          </>
        )}
      </div>
    </div>
    ...
    <div style={{ minHeight: "100vh", background: theme.bg, display: "flex", flexDirection: "column", paddingBottom: 80 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "0 24px" }}>
        {done ? (
          ...
          <>
            <h2 style={{ color: theme.text, fontFamily: "'Bebas Neue', cursive", fontSize: 28, letterSpacing: 2, margin: "0 0 8px" }}>ESCANEAR GYM</h2>
            <p style={{ color: theme.muted, fontSize: 14, marginBottom: 32 }}>Apunta al código QR del gimnasio</p>
    ...
    <div style={{ minHeight: "100vh", background: theme.bg, paddingBottom: 90 }}>
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, margin: "24px 0 24px" }}>
          <div style={{ width: 68, height: 68, borderRadius: "50%", background: `linear-gradient(135deg, ${theme.accent}, ${theme.accentDark})`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 28, color: "#fff", boxShadow: `0 0 20px ${theme.accentGlow}` }}>F</div>
          <div>
            <h2 style={{ color: theme.text, fontSize: 20, fontWeight: 800, margin: 0 }}>Fernando G.</h2>
            <p style={{ color: theme.muted, fontSize: 13, margin: "4px 0 6px" }}>fernando@email.com</p>
    ...
    <div style={{ minHeight: "100vh", background: theme.bg, paddingBottom: 30 }}>
      ...
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <Card style={{ textAlign: "center", padding: 24 }}>
          ...
          <div style={{ width: 120, height: 120, margin: "0 auto", background: "#fff", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 60 }}>▦</div>
...
function AdminDashboard({ onLogout, gyms: savedGyms, setGyms: setSavedGyms }) {
  ...
  const handleAddGym = () => {
    ...
    const newGym = {
      id: Date.now(), name: gymForm.name, address: gymForm.address,
      phone: gymForm.phone, email: gymForm.email, hours: gymForm.hours,
      status: "active", visits: 0, color: theme.blue, rating: 4.5, distance: "—",
      lat, lng
    };
    ...
    setShowAddGym(false);
    showToast(lat ? "✅ Gimnasio guardado con ubicación en mapa" : "✅ Gimnasio guardado (sin coordenadas)");
  };
  const handleApprove = (req) => {
    const newGym = { id: Date.now(), name: req.name, address: req.address, phone: req.phone, email: req.email, hours: req.hours, status: "active", visits: 0, color: theme.blue, rating: 4.5, distance: "—" };
    const updatedGyms = [...savedGyms, newGym];
  ...
  const handleToggle = (id) => {
    const updated = savedGyms.map(g => g.id === id ? { ...g, status: g.status === "active" ? "inactive" : "active" } : g);
    setSavedGyms(updated);
    db.save("redgym-gimnasios", updated);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.bg, paddingBottom: 30 }}>
      ...
      <div style={{ padding: "16px 20px 0" }}>
        ...
        {adminTab === "gyms" && (
          <>
            <Btn onClick={() => setShowAddGym(!showAddGym)} style={{ width: "100%", marginBottom: 16, padding: "12px" }}>{showAddGym ? "✕ Cancelar" : "+ Agregar Gimnasio"}</Btn>
            {showAddGym && (
              ...
              {requests.map(r => (
                <Card key={r.id} style={{ marginBottom: 12, padding: 14, border: `1px solid ${theme.gold}33` }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={() => handleReject(r)} style={{ flex: 1, padding: "10px", border: "none", borderRadius: 8, background: theme.accent + "22", color: theme.accent, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>✕ Rechazar</button>
                  </div>
                </Card>
              ))}
            </>
  ...
  const loadGyms = () => {
    try {
      ...
      if (Array.isArray(saved) && saved.length > 0) {
        ...
        setGyms(initialGyms);
        db.save("redgym-gimnasios", initialGyms);
      }
    } catch(e) { setGyms(initialGyms); }
    setLoading(false);
  };
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh" }}>
        ...
        {!loading && screen === "profile"   && <ProfileScreen onNavigate={nav} onLogout={handleLogout} />}
